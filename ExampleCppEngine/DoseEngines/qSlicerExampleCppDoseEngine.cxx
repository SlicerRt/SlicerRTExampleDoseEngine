...
//----------------------------------------------------------------------------
qSlicerExampleCppDoseEngine::qSlicerExampleCppDoseEngine(QObject* parent)
  : qSlicerAbstractDoseEngine(parent)
{
  this->m_Name = QString("Example C++ engine");
}

//----------------------------------------------------------------------------
qSlicerExampleCppDoseEngine::~qSlicerExampleCppDoseEngine()
{
}

//---------------------------------------------------------------------------
void qSlicerExampleCppDoseEngine::defineBeamParameters()
{
  // Noise level parameter
  this->addBeamParameterSpinBox(
    "Example C++ engine", "NoiseRange", "Noise range (% of Rx):", "Range of noise added to the prescription dose (+- half of the percentage of the Rx dose)",
    0.0, 99.99, 10.0, 1.0, 2 );
}

//---------------------------------------------------------------------------
QString qSlicerExampleCppDoseEngine::calculateDoseUsingEngine(vtkMRMLRTBeamNode* beamNode, vtkMRMLScalarVolumeNode* resultDoseVolumeNode)
{
  if (!beamNode)
  {
  ...
  std::string randomDoseNodeName = std::string(beamNode->GetName()) + "_ExampleCpp";
